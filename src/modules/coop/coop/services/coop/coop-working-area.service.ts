...
@Service()
export class CoopWorkingAreaServices {
  constructor() {}

  async get(isPagination: boolean, limit: number, offset: number, allQuery: object) {
    var queryText: string = "";
    const sql: string = "SELECT * FROM coop.working_area";
    ...
    if (Object.keys(allQuery).length > 0) {
      ...
      queryText = isPagination
        ? "SELECT * FROM coop.working_area ORDER BY id  LIMIT $1 OFFSET $2"
        : "SELECT * FROM coop.working_area ORDER BY id ";
  ...
  async workingAreaBySamityId(samityId: any) {
    const sql = `select * from coop.working_area where samity_id=$1`;
  ...
  async create(data: workingAreaAttrs): Promise<workingAreaAttrs> {
    const { sql, params } = buildInsertSql("coop.working_area", {
      ...data,
    });
  ...
  async update(id: number, data: workingAreaAttrs): Promise<workingAreaAttrs> {
    const { sql, params } = buildUpdateSql("coop.working_area", id, {
      ...data,
    });
    const {
      rows: [updateResult],
  ...
  async delete(id: number): Promise<workingAreaAttrs | null> {
    const sql = `DELETE FROM coop.working_area WHERE id = $1 RETURNING id`;
  ...
  async count(allQuery: object) {
    var queryText: string = "";
    const sql: string = "SELECT COUNT(id) FROM coop.working_area";
    const allQueryValues: any[] = Object.values(allQuery);
    if (Object.keys(allQuery).length > 0) {
      ...
      queryText = "SELECT COUNT(id) FROM coop.working_area";
