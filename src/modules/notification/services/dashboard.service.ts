...
import { DashboardNotification } from "../interfaces/component.interface";

@Service()
export class DashboardNotificationService {
  constructor() {}

  async create({}: DashboardNotification, transaction?: PoolClient) {
    const vConnection = transaction ? transaction : db.getConnection("master");

    const { sql, params } = buildInsertSql("notification.dashboard", {
      ...arguments[0],
    });
  ...
  //   const {
  //     rows: [data],
  //   } = await (await db.getConnection("master")).query(sql, params);
