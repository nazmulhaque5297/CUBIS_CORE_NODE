...
import { SMSNotification } from "../interfaces/component.interface";

@Service()
export class SMSNotificationService {
  constructor() {}

  async create({}: SMSNotification, transaction?: PoolClient) {
    const vConnection = transaction ? transaction : db.getConnection("master");

    const { sql, params } = buildInsertSql("notification.sms", {
      ...arguments[0],
    });
